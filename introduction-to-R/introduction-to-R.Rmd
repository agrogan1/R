...
cover-image: Introduction-to-R-Forest-letter.png
---
...
```{r, fig.cap="Cover Image", fig.height=3.0, fig.margin = TRUE, echo = FALSE}
...
> Note that R uses a forward slash `/` to specify directory paths. R does not understand the use of a backward slash $\backslash$ to specify directories. R uses `~` to refer to the user's (usually your) home directory.
...
dataset <- read_stata("myStatafile.dta") # Stata
...
R seems to make a stronger distinction between these two types of variables than some other statistical software.[^variable_type]  It can thus sometimes be useful to change variables from one type to another:  
...
will give you the correlation of continuous variables `x` and `y`[^notintuitive].
...
runs a regression (linear model) of `y` on `x` and `z`. 

Type `summary(mymodel)` to display the results.
...
[^two_page_Stata]: This document is inspired by my longstanding "Two Page Stata" document: [(PDF)](http://www-personal.umich.edu/~agrogan/stata/TwoPageStata.pdf) [(HTML)](http://www-personal.umich.edu/~agrogan/stata/TwoPageStata.html).
...
[^notintuitive]:  Here is an example where R turns a simple issue into a difficult one, and the syntax is frankly less than elegant, and not intuitive.  I don't have this syntax memorized.  I use `library(Rcmdr)` if I need to test, or create a script for, a correlation.

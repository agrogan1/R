...
if(knitr::opts_knit$get("rmarkdown.pandoc.to") == "html5") {
  ...
  cat("> Hover over the footnotes for additional information.")
...
R has a reputation for being difficult to learn, and a lot of that reputation is deserved.  
...
# Process and Clean Data [^dollarsign] [^factor] [^factoroptions]
...
## Scatterplot [^plotoptions] [^fewerdots]
...
[^factor]: R makes a strong distinction between *continuous* *numeric* variables that measure  scales like mental health or neighborhood safety, and *categorical* *factor variables* that measure non-ordered categories like religious identity or gender identity. Many statistical and graphical procedures are designed to recognize and work with different variable types.

[^factoroptions]: You *don't* need to use all of the options.  e.g. `mydata$w <- factor(mydata$z)` will work just fine. **Changing variables from factor to numeric, and vice versa can sometimes be the simple solution that solves a lot of problems when you are trying to graph your variables.**
...
[^plotoptions]: You *don't* need to use all of the options.  e.g. `plot(mydata$x,mydata$y)` will work just fine.

[^fewerdots]: When scatterplots have fewer dots than you think they should have, often due to "over-printing", adding some random noise, or "jittering" the dots in the scatterplot may help:  `plot(jitter(mydata$y, factor = 5000) ~ mydata$x)`.  Experiment with different sizes of *factor*.
